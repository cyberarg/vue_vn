...
class HNProyectadoController extends Controller
{
   
    public function getListHN_Op($empresa, $db){

        $list = array();
        $itemsDB = DB::connection($db)->select("CALL hnweb_get_op_hn_inter_empresas(".$empresa.");");
    ...
    public function getListHNProyectados(Request $request){
        ...
        $lstHN_Op = $this->getListHN_Op($empresa, $db);

        $lstItemsHN_Proy = $this->getItemProyectado($anio, $empresa, $db, $rbConsolidado, $marca, $filtros, $lstHN_Op);
