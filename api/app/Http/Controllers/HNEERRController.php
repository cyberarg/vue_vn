...
class HNEERRController extends Controller
{
    ...
    public function getListHNEERR(Request $request){
        ...
        $lstItemsHN_EERR = $this->getItemsEERR($anio, $marca, $concesionario, $filtros, $rbConsolidado);
        
        $lstMeses = $this->getEERRMeses($anio, $lstItemsHN_EERR['ListHN']);
        $lstMeses_CE = $this->getEERRMeses($anio, $lstItemsHN_EERR['ListHN_CE']);

        $lstMesesStock = $this->getEERRMesesStock($anio, $lstItemsHN_EERR['ListHN'], $lstMeses['lstEERR'], $lstMeses['SaldoCaja']); 
        $lstMesesStock_CE = $this->getEERRMesesStock($anio, $lstItemsHN_EERR['ListHN_CE'], $lstMeses_CE['lstEERR'], $lstMeses_CE['SaldoCaja']);    

        $lstMesesRent = $this->getEERRMesesRent($anio, $lstItemsHN_EERR['ListHN'], $lstMesesStock['lstStockFinal'], $lstMesesStock['CostoStock'], $lstMesesStock['DurationStock'], $lstMesesStock['DurationStock_TIR']);
        $lstMesesRent_CE = $this->getEERRMesesRent($anio, $lstItemsHN_EERR['ListHN_CE'], $lstMesesStock_CE['lstStockFinal'], $lstMesesStock_CE['CostoStock'], $lstMesesStock_CE['DurationStock'], $lstMesesStock_CE['DurationStock_TIR']);


        $lstEERR['Grid1'] = $lstMeses['lstMeses'];
        $lstEERR['Grid2'] = $lstMesesStock['lstMeseStock'];
        $lstEERR['Grid3'] = $lstMesesRent;

        $lstEERR['Grid1_CE'] = $lstMeses_CE['lstMeses'];
        $lstEERR['Grid2_CE'] = $lstMesesStock_CE['lstMeseStock'];
        $lstEERR['Grid3_CE'] = $lstMesesRent_CE;
    ...
    public function getItemsEERR($anio, $marca, $concesionario, $filtros, $rbConsolidado){
    ...
    public function getEERRMesesRent($anio, $listItems, $listStockFinal, $listCostoStock, $listDurationStock, $listDurationStockTIR){
        $respuesta = array();
        $listStockRent = array();
    ...
    public function getEERRMesesStock($anio, $listItems, $listEERR, $listSaldoCaja){
        ...
        */
        ...
        //for ($i=1; $i < 13; $i++) { 
        for ($i=1; $i <= $mesBorde; $i++) { 
            $lstVerificacion['M'.$i] = ($lstStockFinal['M'.$i] - $lstStockInicial['M'.$i]) - ($listEERR['M'.$i] - $listSaldoCaja['M'.$i]);
        ...
        //EERR del Periodo
        $arrEERRPer['Tipo'] = 'EERR del Período';
        $arrEERRPer['Valores'] = $listEERR;
        array_push($listStock, $arrEERRPer);
    ...
    public function getEERRMeses($anio, $listItems){
        ...
        $lstEERR = array();
        $lstEERR = array_fill_keys($keys, 0);
        ...
        for ($i=1; $i < 13; $i++) { 
           ...
           $lstSaldoCaja['M'.$i] = $lstCobradosReal['M'.$i] - $lstCompradosCosto['M'.$i];
            ...
            $lstEERR['M'.$i] = $lstCompradosCobro['M'.$i] - $lstCompradosCosto['M'.$i] + $lstDifCobro['M'.$i];

            $lstEERR['Total'] +=  $lstEERR['M'.$i];   
        ...
        */
        ...
        $arrEERRPer['Tipo'] = 'EERR del Período';
        $arrEERRPer['Codigo'] = 'EERRPER';
        $arrEERRPer['Valores'] = $lstEERR;
        array_push($listMeses, $arrEERRPer);
